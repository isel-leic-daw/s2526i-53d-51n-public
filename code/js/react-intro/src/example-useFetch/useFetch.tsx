...
export function useFetch(url: string | undefined): UseFetchResult {
  ...
  useEffect(() => {
    ...
    async function doFetch() {
      dispatch({ type: 'start-loading', url: urlToUse });
      try {
        const resp = await fetch(urlToUse, { signal: abortController.signal });
        const json = await resp.json();
        if (!cancelled) {
          dispatch({ type: 'loading-success', url: urlToUse, payload: JSON.stringify(json, null, 2) });
        }
      } catch (error) {
        console.log(`catch error: ${error}`);
        if (!cancelled) {
          dispatch({ type: 'loading-error', error: error as Error });
        }
      }
    }
    doFetch();
