...
type Action =
  | { type: "addCounter"; id: string; label: string }
...
function removeCounter(id: string): Action {
  return {
    type: "removeCounter",
    id: id,
  };
...
function valueUpdate(id: string, newValue: number): Action {
  return {
    type: "valueUpdate",
    id: id,
    newValue: newValue,
  ...
  switch (action.type) {
    case "addCounter":
      return {
        ...
        counters: [
          ...state.counters,
          { id: action.id, label: action.label, value: 0 },
        ],
      };
    ...
    case "removeCounter": {
      ...
      if (!counter) {
        throw new Error("counter id foes not exist");
      }
      const newTotal = state.total - counter.value;
      return {
        total: newTotal,
        counters: state.counters.filter(it => it !== counter),
      };
    }
    ...
    case "valueUpdate": {
      ...
      return {
        total: newTotal,
        counters: state.counters.map(it => it === counter
          ? {...it, value: action.newValue}
          : it
        ),
      };
    }
    ...
    <div>
      ...
      {observedState.counters.map((counter) => (
        <div key={counter.id}>
          <Counter
            label={counter.label}
            onIncrement={(newValue) => dispatch(valueUpdate(counter.id, newValue))}
          />
